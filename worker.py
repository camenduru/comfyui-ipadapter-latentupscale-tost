...
def generate(command):
    ...
    if latent_upscale:
        ...
        return "/content/TotoroUI/models/final_up_image.png"
    else:
        with torch.inference_mode():
            sample = sample[0]["samples"].to(torch.float16)
            vae.first_stage_model.cuda()
        ...
        final_image = Image.fromarray(np.array(decoded*255, dtype=np.uint8)[0])
        final_image.save("/content/TotoroUI/models/final_image.png")
        return "/content/TotoroUI/models/final_image.png"
...
with gr.Blocks(css=".gradio-container {max-width: 544px !important}", analytics_enabled=False) as demo:
    ...
    button.click(fn=generate, inputs=[textbox], outputs=[output_gallery])
